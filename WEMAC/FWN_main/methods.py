...
class wrapper:
    ...
    def ndata_Empatia(self, data, method):
        n_data = numpy.zeros(data.shape)
        n_test = numpy.zeros(data.shape)
        # print(d_dict)
        # print(solution.shape)
        if (method == None):
            print('Method not specified')
            return
        if (method == 'UN'):
            n_data, n_test = data, data
        elif (method == 'MC'):
            n_data, n_test = self.meancenter(data, data)
        elif (method == 'ZS'):
            n_data, n_test = self.zscore(data, data)
        elif (method == 'PS'):
            n_data, n_test = self.paretoscaling(data, data)
        elif (method == 'VSS'):
            n_data, n_test = self.vss(data, data)
        elif (method == 'PT'):
            n_data, n_test = self.power(data, data)
        elif (method == 'MM'):
            n_data, n_test = self.minmax(data, data)
        elif (method == 'MX'):
            n_data, n_test = self.maxnorm(data, data)
        elif (method == 'DS'):
            n_data, n_test = self.decscale(data, data)
        elif (method == 'MD'):
            n_data, n_test = self.mmad(data, data)
        elif (method == 'TH'):
            n_data, n_test = self.hampel(data, data)
        elif (method == 'VTH'):
            n_data, n_test = self.hampelsimple(data, data)
        elif (method == 'SG'):
            n_data, n_test = self.signorm(data, data)
        return n_data, n_test
